...
cameraName = 'Prime'; % Device name in Micro-Manager
...
% Set the camera trigger signal depending on the acquisition type. The
% camera cannot snap widefield images if it's set to 'Trigger First' mode.
if strcmp(g_comBuffer.message, 'snap widefield image')
    g_mmc.setProperty(cameraName, 'TriggerMode', 'Internal Trigger');
else
    g_mmc.setProperty(cameraName, 'TriggerMode', 'Trigger first');
end
